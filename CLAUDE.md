...
**Status (Jul 16):** Treasury is **LIVE** at `https://constellationokx.fly.dev/mcp`, `PAYMENT_MODE=sdk`, and **fully proven end to end** — a paid call settles on-chain and returns its report + receipt. Two confirmed settlements (`0xceaab66…`, `0x87f8674c…`); treasury holds 0.20 USD₮0. Money path hardened over three Codex reviews: durable Postgres receipt store keyed `payer:nonce:tool`, `settle/status` timeout polling + delivery, deliver-on-success, monotonic store, required-resource tool binding, precheck-before-charge. 90 tests green; dashboard wired to the live free path. **Remaining before submit:** redeploy the round-3 fixes (`fly deploy` + `fly scale count 1`); fix the ASP listing per the audit (2-part description with `wallet_id`; represent the 0.1/0.1/0.2 tiers; no paid listing for free tools) and re-audit; activate Agent 5863; sign off 4 INTERFACES items. Repos consolidated on `origin/main`. See `docs/status/P1.md`.
