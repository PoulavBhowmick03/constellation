...
**Status (Jul 15, eve):** Treasury is **LIVE** (`https://constellationokx.fly.dev/mcp`, real x402). Facilitator verify proven on-chain; 77 tests green; revenue math hand-verified on live data. Remaining: funded settlement tx, 4 INTERFACES sign-offs, activate Agent 5863, demo + form. **Repo-split caveat:** the working Treasury impl is on the `migrate/celo` lineage (this side); `origin/main` (constellation.git) uniquely holds `packages/mocks`, `tests/`, `apps/dashboard` on a **separate history**. The two must be reconciled onto one main — I2/AG1 work is NOT lost, it's on origin/main. See `docs/status/P1.md`.
