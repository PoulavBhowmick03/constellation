...
**Status (Jul 16):** Treasury is **LIVE** (`https://constellationokx.fly.dev/mcp`, real x402) and **proven end to end** — paid call settles on-chain and delivers its report. Two confirmed settlements; treasury holds 0.20 USD₮0. Money path hardened over three Codex reviews (durable receipt store keyed `payer:nonce:tool`, timeout polling + delivery, deliver-on-success, monotonic store, required-resource binding, precheck-before-charge); 90 tests green. Dashboard wired to the live free path. Repos consolidated on `origin/main` (mocks/tests/dashboard preserved). Remaining: redeploy round-3 fixes, fix ASP listing per audit + re-audit, activate Agent 5863, demo + form. See `docs/status/P1.md`.
